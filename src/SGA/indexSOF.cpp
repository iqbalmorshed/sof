...
{
    Timer t("sof index");
    parseIndexSOFOptions(argc, argv);
    if(!opt::bDiskAlgo)
    {
        if(opt::algorithm == "sais"){
            //indexInMemorySAIS();
        	std::cout<<"please use ropebwt, sais is unavilable"<<'\n';
        }
        else if(opt::algorithm == "bcr"){
            //indexInMemoryBCR();
        	std::cout<<"please use ropebwt, bcr is unavilable"<<'\n';
        }
        else if(opt::algorithm == "ropebwt")
            indexInMemoryRopebwtRC();
    }
    else
    {
        //indexOnDisk();
    	std::cout<<"please use ropebwt, on disk indexing is unavilable"<<'\n';
    }
    return 0;
}
...
void indexInMemoryRopebwtRC()
{
    ...
    if(opt::bBuildForward)
    {
        std::string bwt_filename = opt::prefix + BWTRC_EXT;
        std::string sai_filename = opt::prefix + SAIRC_EXT;
        BWTCA::runRopebwtRC(opt::readsFile, bwt_filename, use_threads, false);

        if(opt::bBuildSAI)
        {
            std::cout << "\t done bwt construction, generating .bwtrc .sairc file\n";
